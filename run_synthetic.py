...
if __name__ == '__main__':
    ...
    parser.add_argument('--numSeeds', default=20, type=int)                         # number of different seeds
    ...
    parser.add_argument('--runName', default='Synthetic')                        # run name for weights and biases
        ...
        for NY in [50]:
            for l in ls:
                for seed in range(args.numSeeds):
                    ...
                    np.random.seed(seed)
                    if method == 'agnostic':
                        s = agnostic_server.Server(datasets.SyntheticDataset(10,200,10).generate_data(), buildModelClass(11), torch.nn.BCEWithLogitsLoss(), m=None, T=args.numComRnds, lambda_=l, datasetname='Synthetic200', runname=args.runName, device=DEVICE)
                    elif method == 'clientwise':
                        s = clientwise_server.Server(datasets.SyntheticDataset(10,200,10).generate_data(), buildModelClass(11), torch.nn.BCEWithLogitsLoss(), m=None, T=args.numComRnds, lambda_=l, datasetname='Synthetic200', runname=args.runName, device=DEVICE)
                    elif method == 'ours':
                        s = our_server.Server(datasets.SyntheticDataset(10,200,10).generate_data(), buildModelClass(11), torch.nn.BCEWithLogitsLoss(), m=None, T=args.numComRnds, mu=mu, NY=NY, lambda_=l, datasetname='Synthetic200', runname=args.runName, device=DEVICE)
                    elif method== 'fairfed':
                        s = fairfed_server.Server(datasets.SyntheticDataset(10,200,10).generate_data(), buildModelClass(11), torch.nn.BCEWithLogitsLoss(), m=None, beta=beta, T=args.numComRnds, lambda_=l, datasetname='Synthetic200', runname=args.runName, device=DEVICE)
                    else:
