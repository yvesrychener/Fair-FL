        ...
        for NY in [10, 100, 1000]:
            for l in ls:
                for seed in range(NREPE):
                    ...
                    np.random.seed(seed)
                    s = server.Server(datasets.CommunitiesCrimeDataset().load_data(), buildModelClass(99), torch.nn.BCEWithLogitsLoss(), m=None, T=T, mu=mu, NY=NY, lambda_=l, datasetname='CommunitiesCrime', runname='25Sept2023')
                    s.train_test_split()
                    s.sync_N()
                    s.sync_Pa()
                    s.train()
                    ...
                    np.save(f'results/cc_p_{l}_{seed}.npy', performances)
                    np.save(f'results/cc_w_{l}_{seed}.npy', weights)
