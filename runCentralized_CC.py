...
if __name__ == '__main__':
    ...
    for l in ls:
        for seed in range(args.numSeeds):
            ...
            np.random.seed(seed)
            trainer = train.Trainer(datasets.CommunitiesCrimeDataset().load_data(HOMEFOLDER=HOMEFOLDER), buildModelClass(99), torch.nn.BCEWithLogitsLoss(), N_epochs=args.numEpochs, lambda_=l, datasetname='Drug')
            trainer.train()
