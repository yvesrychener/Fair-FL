...
if __name__ == '__main__':
    ...
    NYs = [100] if method=='ours' else [1]
    betas = [0.5, 1.0, 2] if method=='fairfed' else [1] 
    for mu in [1.0]:
        for NY in NYs:
            for beta in betas:
                for l in ls:
                    for seed in range(args.numSeeds):
                        ...
                        np.random.seed(seed)
                        if method == 'agnostic':
                            s = agnostic_server.Server(datasets.CompasDataset().load_data(HOMEFOLDER=HOMEFOLDER), buildModelClass(8), torch.nn.BCEWithLogitsLoss(), m=None, T=args.numComRnds, lambda_=l, datasetname='Compas', runname=args.runName, device=DEVICE)
                        elif method == 'clientwise':
                            s = clientwise_server.Server(datasets.CompasDataset().load_data(HOMEFOLDER=HOMEFOLDER), buildModelClass(8), torch.nn.BCEWithLogitsLoss(), m=None, T=args.numComRnds, lambda_=l, datasetname='Compas', runname=args.runName, device=DEVICE)
                        elif method == 'ours':
                            s = our_server.Server(datasets.CompasDataset().load_data(HOMEFOLDER=HOMEFOLDER), buildModelClass(8), torch.nn.BCEWithLogitsLoss(), m=None, T=args.numComRnds, mu=mu, NY=NY, lambda_=l, datasetname='Compas', runname=args.runName, device=DEVICE, client_stepsize=1e-2)
                        elif method== 'fairfed':
                            s = fairfed_server.Server(datasets.CompasDataset().load_data(HOMEFOLDER=HOMEFOLDER), buildModelClass(8), torch.nn.BCEWithLogitsLoss(), m=None, beta=beta, T=args.numComRnds, lambda_=l, datasetname='Compas', runname=args.runName, device=DEVICE)
                        else:
                        ...
                        np.save(os.path.join(HOMEFOLDER, f'results/{method}/compas_p_{l}_{seed}_{NY}.npy'), performances)
