...
if __name__ == '__main__':
    ...
    NYs = [20, 50, 100, 200] if method=='ours' else [1]
                ...
                for l in ls:
                    for seed in range(args.numSeeds):
                        ...
                        if method == 'agnostic':
                            ...
                            s = our_server.Server(datasets.CommunitiesCrimeDataset().load_data(HOMEFOLDER=HOMEFOLDER), buildModelClass(99), torch.nn.BCEWithLogitsLoss(), m=None, T=args.numComRnds, mu=mu, NY=NY, lambda_=l, datasetname='CommunitiesCrime', runname=args.runName, device=DEVICE, client_stepsize=1e-2)
                        elif method== 'fairfed':
                            s = fairfed_server.Server(datasets.CommunitiesCrimeDataset().load_data(HOMEFOLDER=HOMEFOLDER), buildModelClass(99), torch.nn.BCEWithLogitsLoss(), m=None, beta=beta, T=args.numComRnds, lambda_=l, datasetname='CommunitiesCrime', runname=args.runName, device=DEVICE)
                        else:
